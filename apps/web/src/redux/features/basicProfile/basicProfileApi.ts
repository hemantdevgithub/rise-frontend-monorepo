...
const basicProfileApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getAllBasicProfiles: builder.query({
      query: () => ({
        url: "/basic-profiles//get-all-users",
      }),
      providesTags: ["BasicProfile"],
    }),
    uploadBasicProfileImage: builder.mutation({
      query: (data) => ({
...
export const {
  ...
  useGetAllBasicProfilesQuery,
  useUploadBasicProfileImageMutation,
} = basicProfileApi;
